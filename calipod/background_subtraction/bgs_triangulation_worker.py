...
class BGSTriangulationWorker(QThread):
    ...
    def __init__(self, recording_path: Path, config_path: Path, yolo_xyz_path: Path, bbox=None, region: str = "full"):
        """
    ...
    def run(self):
        ...
        try:
            ...
            self.progress_updated.emit("Processing BGS detections from port-specific labels...")
            ...
            output_path = triangulator.triangulate_bgs_detections_and_save(
                yolo_xyz_path=self.yolo_xyz_path, bbox=self.bbox, region=self.region
            )
            ...
            self.progress_updated.emit("[SUCCESS] Triangulation complete!")
