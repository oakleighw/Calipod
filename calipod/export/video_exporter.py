...
class VideoExportWorker(QObject):
    ...
    def __init__(self, exporter, visualizer, motion_trial, export_start_frame, export_end_frame, output_path):
        super().__init__()
    ...
    def export_motion_video(
        ...
        try:
            ...
            if not collected_frames:
                export_mode = visualizer.export_video_mode
                motion_empty = motion_trial.is_empty if motion_trial else "N/A"
                logger.error(
                    f"No frames were collected to combine into video. "
                    f"Export mode: {export_mode}, motion_trial empty: {motion_empty}"
                )
                return None
            ...
            exported_path = self._render_frames_to_video(collected_frames, output_path, self.video_framerate)
    ...
    def _render_frames_to_video(
        ...
        try:
            ...
            ffmpeg_cmd = [
                ...
                "libx264",
                "-b:v",
                "5000k",  # Reduced from 8000k for stability
                "-preset",
                "faster",  # Changed from medium for more stable real-time encoding
                "-pix_fmt",
                "yuv420p",
                "-movflags",
                "+faststart",
                "-threads",
                "4",  # Limit threads to reduce memory usage
                "-y",
                str(output_path),
            ]
