...
class MetricsComputer:
    ...
    def compute_and_display(self, motion_trial, use_filtered: bool):
        ...
        if not hasattr(motion_trial, "predictions_df") or motion_trial.predictions_df.empty:
            QMessageBox.warning(self.parent, "No Predictions", "Predictions are empty; load predictions before computing metrics.")
            return

        if not hasattr(motion_trial, "xyz_df") or motion_trial.xyz_df.empty:
            QMessageBox.warning(self.parent, "No Ground Truth", "Ground-truth xyz data are empty; cannot compute metrics.")
            return
        ...
        try:
            ...
            metrics_by_source = {}
            if use_filtered and hasattr(motion_trial, 'predictions_df') and 'measurement_source' in motion_trial.predictions_df.columns:
                for source in ['YOLO', 'BGS', 'filter_only']:
                    ...
                    if not source_df.empty:
                        ...
                        try:
                            ...
                            source_metrics = self._compute_metrics_from_dataframes(motion_trial.predictions_df, motion_trial.xyz_df)
    ...
    def _compute_metrics_from_dataframes(self, pred_df: pd.DataFrame, gt_df: pd.DataFrame) -> dict:
        ...
        if merged_df.empty:
            logger.debug("No matching points between ground truth and predictions; returning empty performance metrics.")
            return {}
        ...
        logger.info(f"Filtered GT from {len(gt_df)} to {len(gt_filtered)} rows (only point_ids: {sorted(relevant_point_ids)})")
    ...
    def _display_metrics_dialog(self, metrics: dict, metrics_by_source: dict, motion_trial=None):
        ...
        if motion_trial is not None:
            pred_min_frame = motion_trial.predictions_df['sync_index'].min() if not motion_trial.predictions_df.empty else 0
            pred_max_frame = motion_trial.predictions_df['sync_index'].max() if not motion_trial.predictions_df.empty else 0
            ...
            gt_fly_df = motion_trial.xyz_df[motion_trial.xyz_df['point_id'] == 0] if not motion_trial.xyz_df.empty else pd.DataFrame()
            total_gt_fly_points = len(gt_fly_df) if not gt_fly_df.empty else 0
            # Only count pred points for point_id == 0 (the tracked fly)
            pred_fly_df = motion_trial.predictions_df[motion_trial.predictions_df['point_id'] == 0] if not motion_trial.predictions_df.empty else pd.DataFrame()
